...
public class PsoScheduling {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String vmm = "Xen";
			for (int i = 0; i < Constant.NO_OF_DATA_CENTERS; i++) {
				vmlist.add(new Vm(datacenter[i].getId(), brokerId, mips, pesNumber, ram, bw, size, vmm,
			...
			for (int i = 0; i < Constant.NO_OF_TASKS; i++) {
				Cloudlet cloudlet1 = new Cloudlet(i,
						(int) (1e3 * (execTimeMatrix[i][(int) mapping[i]] + communTimeMatrix[i][(int) mapping[i]])), pesNumber,
			...
			for (int i = 0; i < mapping.length; i++) {
				if (hm.containsKey((int) mapping[i]))
					continue;
